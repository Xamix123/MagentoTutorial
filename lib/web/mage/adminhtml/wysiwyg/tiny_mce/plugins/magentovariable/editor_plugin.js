        ...
        tinymce.create('tinymce.plugins.magentovariable', {
            ...
            init: function (editor, url) {
                ...
                editor.addButton('magentovariable', {
                    ...
                    /**
                     * Highlight or dismiss Insert Variable button when variables is selected or deselected.
                     */
                    onPostRender: function () {
                        ...
                        toggleVariableButton = function (e) {
                            ...
                            if (jQuery(e.target).hasClass('magento-variables')) {
                                _this.active(true);
                            }
                        };
                ...
                /**
                 * Double click handler on the editor to handle dbl click on variables placeholder.
                 */
                editor.on('dblclick', function (evt) {
                    if (jQuery(evt.target).hasClass('magento-variables')) {
            ...
            encodeVariables: function (content) {
                content = content.gsub(/\{\{config path=\"([^\"]+)\"\}\}/i, function (match) {
                    ...
                    if (magentoVariables[match[1]] && magentoVariables[match[1]]['variable_type'] === 'default') {
                        imageHtml = '<span id="%id" class="magento-variables magento-placeholder mceNonEditable">' +
                            '%s</span>';
                        imageHtml = imageHtml.replace('%s', magentoVariables[match[1]]['variable_name']);
                    } else {
                        imageHtml = '<span id="%id" class="' +
                            'magento-variables magento-placeholder magento-placeholder-error ' +
                            'mceNonEditable' +
                            '">' +
                            'Not found' +
                            '</span>';
                ...
                content = content.gsub(/\{\{customVar code=([^\}\"]+)\}\}/i, function (match) {
                    ...
                    if (magentoVariables[match[1]] && magentoVariables[match[1]]['variable_type'] === 'custom') {
                        imageHtml = '<span id="%id" class="magento-variables magento-custom-var magento-placeholder ' +
                            'mceNonEditable">%s</span>';
                        imageHtml = imageHtml.replace('%s', magentoVariables[match[1]]['variable_name']);
                    } else {
                        imageHtml = '<span id="%id" class="' +
                            'magento-variables magento-custom-var magento-placeholder ' +
            ...
            decodeVariables: function (content) {
                ...
                [].forEach.call(doc.querySelectorAll('span.magento-variables'), function (el) {
                    var $el = jQuery(el);
