...
class HtmlDirectiveSniff implements Sniff
{
    ...
    /**
     * Validate directive variables usage is valid. e.g. {{var <variables body>}} or {{somedir some_param="$foo.bar()"}}
     *
     * @param File $phpcsFile
     * @param string $body
     */
    ...
    private function validateDefinedVariables(File $phpcsFile, string $templateText): void
    {
            ...
            foreach ($definedVariables as $var => $label) {
                if (empty($label)) {
                    $phpcsFile->addError(
                        ...
                        . 'Label for variables "' . $var . '" is empty.',
                        null,
        ...
        foreach ($undefinedVariables as $undefinedVariable) {
            $phpcsFile->addError(
                'Template @vars comment block is missing a variables used in the template.' . PHP_EOL
                 . 'Missing variables: ' . $undefinedVariable,
                null,
